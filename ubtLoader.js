...
async function readFiles(dir) {
    ...
    if (ast && ast.childNodes) {
        for (const child of ast.childNodes) {
            if(child.nodeName!=='#text'){
                if(child.attrs.find(item=>item?.name==='@click')){
                    clickList.push(child)
                }
                const cpn = imports.find(item1=>normalizeCpnName(child.tagName)===normalizeCpnName(item1.name))
                if(cpn){
                    addCpnToRoutes(cpn)
                }
            }
  ...
  async function visitAst(tempalteCode,scriptCode){
    const imports = [];
        ...
        const templateAst = parse(tempalteCode);
        console.log(45555,templateAst.childNodes[0].childNodes[1].childNodes[0])
...
function findTextNode(node){
    ...
    for(const child of node.childNodes){
        if(child.value&&child.value.trim() !== ''){
            return child.value
        }else if(child.childNodes&&child.childNodes[0]&&child.childNodes[0].value){
  ...
  function addDataUbtidToClickElements(templateContent) {
    ...
    // const modifiedHtml = serialize(ast.childNodes[0].childNodes[1].childNodes[0].childNodes[1]);
    // console.log(323,modifiedHtml)
    ...
    if(clickList&&routes){
        clickList.forEach((node)=>{
            const clickelement = serializeOuter(node)
            // console.log(32122,templateContent.includes(clickelement))
            const rawContent = findTextNode(node)
            content = rawContent?.replace(/\n/g, '')?.replace(/\r/g, '')?.replace(/\s/g, '')||''
            clickContentList.push(content)
            const ubtidValue = node.attrs.find(item=>item.name==='data-ubtid')?.value||''
            const ubtkeyValue = node.attrs.find(item=>item.name==='data-ubtKey')?.value||''
        })
    //     clickList.forEach((node)=>{
    //         let content 
